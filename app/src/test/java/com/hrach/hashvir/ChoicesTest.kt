...
class ChoicesTest {
    ...
    @Test
    fun `four distinct choices in range, always containing the answer`() {
        val random = Random(7)
        for (answer in 1..10) {
            repeat(200) {
                val choices = choicesFor(answer, random)
                assertEquals("$answer -> $choices", 4, choices.size)
                assertEquals("$answer -> $choices not distinct", 4, choices.toSet().size)
    ...
    @Test
    fun `distractors stay near the answer`() {
        val random = Random(8)
        for (answer in 1..10) {
            repeat(200) {
                ...
                assertTrue(
                    ...
                    distractors.all { abs(it - answer) in 1..6 },
                )
            }
        }
    }
