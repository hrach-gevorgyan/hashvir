#!/usr/bin/env python3
"""
Generate Armenian voice clips for Հաշվի՛ր.
...
Every clip is Պույ-պույ. He is a mouse, so the voice is pitched up and slowed
...
Shared with the puy-puy app, which is where this script came from. Keep the two
copies in step: a voice that drifts between the apps stops being one character.

The recorded clips already in res/raw are a real human voice and are better than
anything here. Existing files are skipped, so this fills gaps rather than
replacing them; pass --force only if you mean to lose a recording.
...
def main():
    ...
    ap.add_argument("--words", type=Path, default=ROOT / "tools" / "words.csv")
    ap.add_argument("--out", type=Path, default=ROOT / "app" / "src" / "main" / "res" / "raw")
